...
class MarketIndexDirectTest < Minitest::Test
  def test_direct_load_market_index
    setup = market_index_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-market_index", setup[:live] ? "live" : "unit")
    if _should_skip
...
def market_index_direct_setup(mockres)
