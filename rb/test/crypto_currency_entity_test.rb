...
class CryptoCurrencyEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = crypto_currency_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "crypto_currency." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FINANCIAL_DATA_TEST_CRYPTO_CURRENCY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    crypto_currency_ref01_data = nil
    if crypto_currency_ref01_data_raw.length > 0
...
def crypto_currency_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "crypto_currency", "CryptoCurrencyTestData.json")
  ...
  entid_env_raw = ENV["FINANCIAL_DATA_TEST_CRYPTO_CURRENCY_ENTID"]
