...
class SymbolListEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = FinancialDataSDK.test(seed, nil)
    seen = base.SymbolList(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.SymbolList(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = symbol_list_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "symbol_list." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FINANCIAL_DATA_TEST_SYMBOL_LIST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    symbol_list_ref01_data = nil
    if symbol_list_ref01_data_raw.length > 0
...
def symbol_list_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "symbol_list", "SymbolListTestData.json")
  ...
  entid_env_raw = ENV["FINANCIAL_DATA_TEST_SYMBOL_LIST_ENTID"]
