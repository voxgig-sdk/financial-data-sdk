...
class FinancialStatementEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FINANCIALDATA_TEST_FINANCIAL_STATEMENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    financial_statement_ref01_data_dt0_loaded, err = financial_statement_ref01_ent.load(financial_statement_ref01_match_dt0, nil)
    assert_nil err
...
def financial_statement_basic_setup(extra)
  ...
  entid_env_raw = ENV["FINANCIALDATA_TEST_FINANCIAL_STATEMENT_ENTID"]
