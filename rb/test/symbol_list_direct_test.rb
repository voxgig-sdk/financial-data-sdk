...
class SymbolListDirectTest < Minitest::Test
  def test_direct_list_symbol_list
    setup = symbol_list_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-symbol_list", setup[:live] ? "live" : "unit")
    if _should_skip
...
def symbol_list_direct_setup(mockres)
