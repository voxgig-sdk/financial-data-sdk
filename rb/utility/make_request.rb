...
module FinancialDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FinancialDataResponse.new({})
    ...
    if fetch_err
      ...
      response = FinancialDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FinancialDataResponse.new(fetched)
    else
