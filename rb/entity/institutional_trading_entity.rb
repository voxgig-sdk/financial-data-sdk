...
class InstitutionalTradingEntity
  ...
  def make
    opts = @_entopts.dup
    InstitutionalTradingEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FinancialDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [InstitutionalTrading, Hash] the current InstitutionalTrading data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FinancialDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of InstitutionalTrading fields)
  def match_get
  ...
  #
  # @param reqmatch [InstitutionalTradingLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.InstitutionalTrading.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [InstitutionalTrading, Hash] the loaded InstitutionalTrading; raises FinancialDataError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FinancialDataHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
