...
class ForexDataEntity
  ...
  def make
    opts = @_entopts.dup
    ForexDataEntity.new(@_client, opts)
  end
  ...
  # @return [ForexData, Hash] the current ForexData data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of ForexData fields)
  def match_get
  ...
  #
  # @param reqmatch [ForexDataLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ForexData.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ForexData, Hash] the loaded ForexData; raises FinancialDataError on failure
