// Command FinancialData-cli is a CLI + REPL for the FinancialData SDK,
...
const entitiesHelp = "basic_information crypto_currency derivatives_data esg_data etf_data event_calendar financial_ratio financial_statement forex_data insider_trading institutional_trading investment_adviser market_data market_index market_new miscellaneous_data mutual_fund symbol_list"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FINANCIAL_DATA_APIKEY (api key), FINANCIAL_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FINANCIAL_DATA_APIKEY carries the API key and
	// FINANCIAL_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FINANCIAL_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FINANCIAL_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFinancialDataSDK(opts)
