...
boru-driven command-line client **and** interactive REPL for the FinancialData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/financial-data-cli)
make build

# 2. See usage (words, entities, env vars)
./financial-data-cli --help

# 3. Provide credentials once, via the environment
export FINANCIAL_DATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./financial-data-cli load 1 basic_information            # {id:1} shorthand
./financial-data-cli load '{id:1}' basic_information       # explicit match map

# 5. Override the API base URL for a single call
FINANCIAL_DATA_BASE=https://api.example.com ./financial-data-cli load 1 basic_information

# 6. No arguments -> interactive REPL
./financial-data-cli
financial-data> load 1 basic_information
financial-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/financial-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/financial-data-cli load 1 basic_information
   ```
...
```sh
./financial-data-cli load 1 basic_information          # scalar shorthand for {id:1}
./financial-data-cli load '{id:1}' basic_information     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `financial-data>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `basic_information`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/financial-data-cli`. |
...
(`./financial-data-cli <expr>`) and interactively (the REPL), and expressions compose the
