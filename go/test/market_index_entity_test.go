...
func TestMarketIndexEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		marketIndexRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.market_index")))
...
func market_indexBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FINANCIAL_DATA_TEST_MARKET_INDEX_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FINANCIAL_DATA_TEST_MARKET_INDEX_ENTID"])
