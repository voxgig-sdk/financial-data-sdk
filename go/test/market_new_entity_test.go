...
func TestMarketNewEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		marketNewRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.market_new")))
...
func market_newBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FINANCIAL_DATA_TEST_MARKET_NEW_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FINANCIAL_DATA_TEST_MARKET_NEW_ENTID"])
