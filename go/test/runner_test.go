...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FINANCIAL_DATA_TEST_LIVE") == "TRUE" ||
		os.Getenv("FINANCIAL_DATA_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FINANCIAL_DATA_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.FinancialDataSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.FinancialDataError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.FinancialDataSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.FinancialDataError{Msg: msg, Code: code}
}
