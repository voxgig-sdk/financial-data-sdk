...
func TestMarketDataEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.MarketData(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.MarketData(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FINANCIAL_DATA_TEST_MARKET_DATA_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func market_dataBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FINANCIAL_DATA_TEST_MARKET_DATA_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FINANCIAL_DATA_TEST_MARKET_DATA_ENTID"])
