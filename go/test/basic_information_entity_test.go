...
func TestBasicInformationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FINANCIALDATA_TEST_BASIC_INFORMATION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func basic_informationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FINANCIALDATA_TEST_BASIC_INFORMATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FINANCIALDATA_TEST_BASIC_INFORMATION_ENTID"])
