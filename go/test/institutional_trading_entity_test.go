...
func TestInstitutionalTradingEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FINANCIAL_DATA_TEST_INSTITUTIONAL_TRADING_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		institutionalTradingRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.institutional_trading")))
...
func institutional_tradingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FINANCIAL_DATA_TEST_INSTITUTIONAL_TRADING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FINANCIAL_DATA_TEST_INSTITUTIONAL_TRADING_ENTID"])
