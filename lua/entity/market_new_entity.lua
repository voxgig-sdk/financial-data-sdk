...
local MarketNewEntity = {}
MarketNewEntity.__index = MarketNewEntity


function MarketNewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MarketNewEntity)
  self._name = "market_new"
  self._client = client
...
function MarketNewEntity:get_name()
  return self._name
end


function MarketNewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MarketNewEntity.new(self._client, opts)
end
...
function MarketNewEntity:mark_deleted()
  self._deleted = true
end


function MarketNewEntity:deleted()
  return true == self._deleted
end


function MarketNewEntity:data_set(args)
  if args ~= nil then
...
function MarketNewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MarketNewEntity:match_set(args)
  if args ~= nil then
...
function MarketNewEntity:match_get()
...
function MarketNewEntity:stream(action, args, callopts)
...
---@param reqmatch MarketNewLoadMatch
---@param ctrl? table
---@return MarketNew
---@return string? err
function MarketNewEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function MarketNewEntity:_run_op(ctx, post_done)
  local utility = self._utility
