...
local ForexDataEntity = {}
ForexDataEntity.__index = ForexDataEntity


function ForexDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ForexDataEntity)
  self._name = "forex_data"
  self._client = client
...
function ForexDataEntity:get_name()
  return self._name
end


function ForexDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ForexDataEntity.new(self._client, opts)
end
...
function ForexDataEntity:mark_deleted()
  self._deleted = true
end


function ForexDataEntity:deleted()
  return true == self._deleted
end


function ForexDataEntity:data_set(args)
  if args ~= nil then
...
function ForexDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ForexDataEntity:match_set(args)
  if args ~= nil then
...
function ForexDataEntity:match_get()
...
function ForexDataEntity:stream(action, args, callopts)
...
function ForexDataEntity:_run_op(ctx, post_done)
  local utility = self._utility
