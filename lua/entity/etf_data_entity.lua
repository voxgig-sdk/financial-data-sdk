...
local EtfDataEntity = {}
EtfDataEntity.__index = EtfDataEntity


function EtfDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EtfDataEntity)
  self._name = "etf_data"
  self._client = client
...
function EtfDataEntity:get_name()
  return self._name
end


function EtfDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EtfDataEntity.new(self._client, opts)
end
...
function EtfDataEntity:mark_deleted()
  self._deleted = true
end


function EtfDataEntity:deleted()
  return true == self._deleted
end


function EtfDataEntity:data_set(args)
  if args ~= nil then
...
function EtfDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EtfDataEntity:match_set(args)
  if args ~= nil then
...
function EtfDataEntity:match_get()
...
function EtfDataEntity:stream(action, args, callopts)
...
function EtfDataEntity:_run_op(ctx, post_done)
  local utility = self._utility
