...
local MarketIndexEntity = {}
MarketIndexEntity.__index = MarketIndexEntity


function MarketIndexEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MarketIndexEntity)
  self._name = "market_index"
  self._client = client
...
function MarketIndexEntity:get_name()
  return self._name
end


function MarketIndexEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MarketIndexEntity.new(self._client, opts)
end
...
function MarketIndexEntity:mark_deleted()
  self._deleted = true
end


function MarketIndexEntity:deleted()
  return true == self._deleted
end


function MarketIndexEntity:data_set(args)
  if args ~= nil then
...
function MarketIndexEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MarketIndexEntity:match_set(args)
  if args ~= nil then
...
function MarketIndexEntity:match_get()
...
function MarketIndexEntity:stream(action, args, callopts)
...
---@param reqmatch MarketIndexLoadMatch
---@param ctrl? table
---@return MarketIndex
---@return string? err
function MarketIndexEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function MarketIndexEntity:_run_op(ctx, post_done)
  local utility = self._utility
