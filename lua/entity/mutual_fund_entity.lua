...
local MutualFundEntity = {}
MutualFundEntity.__index = MutualFundEntity


function MutualFundEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MutualFundEntity)
  self._name = "mutual_fund"
  self._client = client
...
function MutualFundEntity:get_name()
  return self._name
end


function MutualFundEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MutualFundEntity.new(self._client, opts)
end
...
function MutualFundEntity:mark_deleted()
  self._deleted = true
end


function MutualFundEntity:deleted()
  return true == self._deleted
end


function MutualFundEntity:data_set(args)
  if args ~= nil then
...
function MutualFundEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MutualFundEntity:match_set(args)
  if args ~= nil then
...
function MutualFundEntity:match_get()
...
function MutualFundEntity:stream(action, args, callopts)
...
function MutualFundEntity:_run_op(ctx, post_done)
  local utility = self._utility
