...
local EsgDataEntity = {}
EsgDataEntity.__index = EsgDataEntity


function EsgDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EsgDataEntity)
  self._name = "esg_data"
  self._client = client
...
function EsgDataEntity:get_name()
  return self._name
end


function EsgDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EsgDataEntity.new(self._client, opts)
end
...
function EsgDataEntity:mark_deleted()
  self._deleted = true
end


function EsgDataEntity:deleted()
  return true == self._deleted
end


function EsgDataEntity:data_set(args)
  if args ~= nil then
...
function EsgDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EsgDataEntity:match_set(args)
  if args ~= nil then
...
function EsgDataEntity:match_get()
...
function EsgDataEntity:stream(action, args, callopts)
...
---@param reqmatch EsgDataLoadMatch
---@param ctrl? table
---@return EsgData
---@return string? err
function EsgDataEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function EsgDataEntity:_run_op(ctx, post_done)
  local utility = self._utility
