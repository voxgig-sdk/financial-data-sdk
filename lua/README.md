# FinancialData Lua SDK

The Lua SDK for the FinancialData API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:BasicInformation(nil):load({ id = "example_id" }, nil)
...
local result, err = client:FinancialData(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── financial-data_sdk.lua    -- Main SDK module
...
The main module (`financial-data_sdk`) exports the SDK constructor
