...
class TestSymbolListDirect:

    def test_should_direct_list_symbol_list(self):
        setup = _symbol_list_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-symbol_list", "live" if setup["live"] else "unit")
        if _skip:
...
def _symbol_list_direct_setup(mockres):
