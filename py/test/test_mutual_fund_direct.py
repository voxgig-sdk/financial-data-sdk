...
class TestMutualFundDirect:

    def test_should_direct_load_mutual_fund(self):
        setup = _mutual_fund_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-mutual_fund", "live" if setup["live"] else "unit")
        if _skip:
...
def _mutual_fund_direct_setup(mockres):
