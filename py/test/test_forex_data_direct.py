...
class TestForexDataDirect:

    def test_should_direct_load_forex_data(self):
        setup = _forex_data_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-forex_data", "live" if setup["live"] else "unit")
        if _skip:
...
def _forex_data_direct_setup(mockres):
