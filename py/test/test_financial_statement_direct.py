...
class TestFinancialStatementDirect:

    def test_should_direct_load_financial_statement(self):
        setup = _financial_statement_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-financial_statement", "live" if setup["live"] else "unit")
        if _skip:
...
def _financial_statement_direct_setup(mockres):
