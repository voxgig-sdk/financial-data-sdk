...
class TestMarketNewDirect:

    def test_should_direct_load_market_new(self):
        setup = _market_new_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-market_new", "live" if setup["live"] else "unit")
        if _skip:
...
def _market_new_direct_setup(mockres):
