...
class TestEtfDataDirect:

    def test_should_direct_load_etf_data(self):
        setup = _etf_data_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-etf_data", "live" if setup["live"] else "unit")
        if _skip:
...
def _etf_data_direct_setup(mockres):
