...
class TestEventCalendarDirect:

    def test_should_direct_load_event_calendar(self):
        setup = _event_calendar_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-event_calendar", "live" if setup["live"] else "unit")
        if _skip:
...
def _event_calendar_direct_setup(mockres):
