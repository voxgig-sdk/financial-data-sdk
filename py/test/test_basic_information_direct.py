...
class TestBasicInformationDirect:

    def test_should_direct_load_basic_information(self):
        setup = _basic_information_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-basic_information", "live" if setup["live"] else "unit")
        if _skip:
...
def _basic_information_direct_setup(mockres):
