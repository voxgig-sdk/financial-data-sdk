...
class TestInsiderTradingDirect:

    def test_should_direct_load_insider_trading(self):
        setup = _insider_trading_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-insider_trading", "live" if setup["live"] else "unit")
        if _skip:
...
def _insider_trading_direct_setup(mockres):
