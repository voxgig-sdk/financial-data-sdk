...
class TestEtfDataEntity:

    def test_should_create_instance(self):
        testsdk = FinancialDataSDK.test(None, None)
        ent = testsdk.EtfData(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _etf_data_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "etf_data." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FINANCIAL_DATA_TEST_ETF_DATA_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        etf_data_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.etf_data")))
        etf_data_ref01_data = None
        if len(etf_data_ref01_data_raw) > 0:
...
def _etf_data_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/etf_data/EtfDataTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "FINANCIAL_DATA_TEST_ETF_DATA_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("FINANCIAL_DATA_TEST_ETF_DATA_ENTID"))
