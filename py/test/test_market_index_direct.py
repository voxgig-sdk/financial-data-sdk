...
class TestMarketIndexDirect:

    def test_should_direct_load_market_index(self):
        setup = _market_index_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-market_index", "live" if setup["live"] else "unit")
        if _skip:
...
def _market_index_direct_setup(mockres):
