...
class CryptoCurrencyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CryptoCurrencyEntity(self._client, opts)
    ...
    def data_get(self) -> CryptoCurrency:
    ...
    def match_get(self) -> CryptoCurrency:
    ...
    def load(self, reqmatch=None, ctrl=None) -> CryptoCurrency:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.CryptoCurrency().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
