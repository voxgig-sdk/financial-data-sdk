...
The Python SDK for the FinancialData API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.BasicInformation(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.FinancialData(None).load(
    {"id": "test01"}, None
)
