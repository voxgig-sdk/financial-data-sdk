...
// FinancialData SDK pipeline test
//
...
{
    private static function utility(): FinancialDataUtility
    {
        return new FinancialDataUtility();
    }
    ...
    private static function ctx(array $over = []): FinancialDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FinancialDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FinancialDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FinancialDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FinancialDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FinancialDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FinancialDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FinancialDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FinancialDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FinancialDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FinancialDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FinancialDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FinancialDataResponse([]),
            'result' => new FinancialDataResult([]),
        ]);
        [, $err] = FinancialDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FinancialDataSpec([]),
            'response' => null,
            'result' => new FinancialDataResult([]),
        ]);
        [, $err] = FinancialDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FinancialDataSpec(['step' => 's']),
            'response' => new FinancialDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FinancialDataResult([]),
        ]);
        [, $err] = FinancialDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FinancialDataSpec(['step' => 's']),
            'response' => new FinancialDataResponse(self::resp(200, ['v' => 1])),
            'result' => new FinancialDataResult([]),
        ]);
        [, $err] = FinancialDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FinancialDataSpec(['step' => 's']),
            'response' => new FinancialDataResponse(self::resp(200, ['v' => 2])),
            'result' => new FinancialDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FinancialDataMakeResponse::call($ctx);
    ...
    {
        $preset = new FinancialDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FinancialDataSpec([]),
            'response' => new FinancialDataResponse([]),
            'result' => new FinancialDataResult([]),
        ...
        [$got, $err] = FinancialDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FinancialDataResult([])]);
        [, $err] = FinancialDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FinancialDataSpec([]), 'result' => null]);
        [, $err] = FinancialDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FinancialDataSpec(['step' => 's']),
            'result' => new FinancialDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FinancialDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FinancialDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FinancialDataSpec(['step' => 's']),
            'result' => new FinancialDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FinancialDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FinancialDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FinancialDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FinancialDataSpec([]),
            'result' => new FinancialDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FinancialDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FinancialDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FinancialDataError('boom', 'boom');
        $utility->fetcher = function (FinancialDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FinancialDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FinancialDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FinancialDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FinancialDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FinancialDataResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        FinancialDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FinancialDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FinancialDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FinancialDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FinancialDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FinancialDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FinancialDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FinancialDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FinancialDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FinancialDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FinancialDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FinancialDataResult(['ok' => false])]);
        $this->expectException(FinancialDataError::class);
        FinancialDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FinancialDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FinancialDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FinancialDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FinancialDataBaseFeature();
        $b = new FinancialDataBaseFeature();
        FinancialDataFeatureAdd::call($ctx, $a);
        FinancialDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FinancialDataBaseFeature
    {
        $f = new FinancialDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FinancialDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FinancialDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FinancialDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FinancialDataPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        FinancialDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FinancialDataResponse(['headers' => null]),
            'result' => new FinancialDataResult([]),
        ]);
        FinancialDataResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        FinancialDataResultBody::call($ctx);
