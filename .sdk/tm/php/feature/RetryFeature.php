...
// FinancialData SDK retry feature
...
class FinancialDataRetryFeature extends FinancialDataBaseFeature
{
    ...
    public function init(FinancialDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FinancialDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FinancialDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FinancialDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
