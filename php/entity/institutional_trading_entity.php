...
// FinancialData SDK InstitutionalTrading entity
...
class InstitutionalTradingEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "institutional_trading";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new InstitutionalTradingEntity($this->_client, $opts);
    }

    /**
     * @param InstitutionalTrading|array $args InstitutionalTrading data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = FinancialDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return InstitutionalTrading|array The current InstitutionalTrading data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of InstitutionalTrading fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = FinancialDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of InstitutionalTrading fields).
     */
    ...
    /**
     * Load a single InstitutionalTrading.
     *
     * @param InstitutionalTradingLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed InstitutionalTradingLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return InstitutionalTrading|array The loaded InstitutionalTrading as an assoc-array at the
     *   SDK boundary; throws FinancialDataError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = FinancialDataHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
