...
class FinancialDataMakeError
{
    public static function call(?FinancialDataContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FinancialDataContext([], null);
        }
        $op = $ctx->op ?? new FinancialDataOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FinancialDataResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FinancialDataError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FinancialDataError('', $msg, $ctx);
        ...
        if ($err instanceof FinancialDataError) {
