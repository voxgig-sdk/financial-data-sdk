...
// FinancialData SDK feature test
//
...
class FtCtrl extends FinancialDataControl
{
...
class FtClient
{
    ...
    private FinancialDataUtility $utility;

    public function __construct(FinancialDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): FinancialDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FinancialDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FinancialDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FinancialDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FinancialDataConfig::make_feature($name);
    ...
    private static function buildUrl(FinancialDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FinancialDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FinancialDataResponse($fetched);
        }
    ...
    private function fail(FinancialDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FinancialDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
