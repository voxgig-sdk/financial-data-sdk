...
class BasicInformationEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = FinancialDataSDK::test(null, null);
        $ent = $testsdk->BasicInformation(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = basic_information_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "basic_information." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FINANCIALDATA_TEST_BASIC_INFORMATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $basic_information_ref01_data = null;
        if (count($basic_information_ref01_data_raw) > 0) {
...
function basic_information_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/basic_information/BasicInformationTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["basic_information01", "basic_information02", "basic_information03"] as $k) {
    ...
    $entid_env_raw = getenv("FINANCIALDATA_TEST_BASIC_INFORMATION_ENTID");
