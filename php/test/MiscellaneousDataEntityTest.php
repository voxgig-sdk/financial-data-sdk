...
class MiscellaneousDataEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = FinancialDataSDK::test(null, null);
        $ent = $testsdk->MiscellaneousData(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = miscellaneous_data_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "miscellaneous_data." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FINANCIAL_DATA_TEST_MISCELLANEOUS_DATA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $miscellaneous_data_ref01_data = null;
        if (count($miscellaneous_data_ref01_data_raw) > 0) {
...
function miscellaneous_data_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/miscellaneous_data/MiscellaneousDataTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["miscellaneous_data01", "miscellaneous_data02", "miscellaneous_data03"] as $k) {
    ...
    $entid_env_raw = getenv("FINANCIAL_DATA_TEST_MISCELLANEOUS_DATA_ENTID");
