...
class CryptoCurrencyEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = FinancialDataSDK::test(null, null);
        $ent = $testsdk->CryptoCurrency(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = crypto_currency_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "crypto_currency." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FINANCIAL_DATA_TEST_CRYPTO_CURRENCY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $crypto_currency_ref01_data = null;
        if (count($crypto_currency_ref01_data_raw) > 0) {
...
function crypto_currency_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/crypto_currency/CryptoCurrencyTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["crypto_currency01", "crypto_currency02", "crypto_currency03"] as $k) {
    ...
    $entid_env_raw = getenv("FINANCIAL_DATA_TEST_CRYPTO_CURRENCY_ENTID");
