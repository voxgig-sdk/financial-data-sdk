...
class MutualFundEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = FinancialDataSDK::test(null, null);
        $ent = $testsdk->MutualFund(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = mutual_fund_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "mutual_fund." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FINANCIALDATA_TEST_MUTUAL_FUND_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $mutual_fund_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.mutual_fund")));
        $mutual_fund_ref01_data = null;
        if (count($mutual_fund_ref01_data_raw) > 0) {
            $mutual_fund_ref01_data = Helpers::to_map($mutual_fund_ref01_data_raw[0][1]);
        }
...
function mutual_fund_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/mutual_fund/MutualFundTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["mutual_fund01", "mutual_fund02", "mutual_fund03"] as $k) {
    ...
    $entid_env_raw = getenv("FINANCIALDATA_TEST_MUTUAL_FUND_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["FINANCIALDATA_TEST_MUTUAL_FUND_ENTID"]);
