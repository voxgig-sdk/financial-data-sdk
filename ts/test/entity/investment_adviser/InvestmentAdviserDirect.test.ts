...
describe('InvestmentAdviserDirect', async () => {
  ...
  test('direct-load-investment_adviser', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-investment_adviser', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FINANCIAL_DATA_TEST_INVESTMENT_ADVISER_ENTID']
