...
describe('FinancialStatementDirect', async () => {
  ...
  test('direct-load-financial_statement', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-financial_statement', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FINANCIAL_DATA_TEST_FINANCIAL_STATEMENT_ENTID']
