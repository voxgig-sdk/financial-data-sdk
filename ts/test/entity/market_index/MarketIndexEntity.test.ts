...
describe('MarketIndexEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'market_index.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set FINANCIAL_DATA_TEST_MARKET_INDEX_ENTID JSON to run live')
      return
    }
    ...
    const market_index_ref01_data_dt0 = await market_index_ref01_ent.load(market_index_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['FINANCIAL_DATA_TEST_MARKET_INDEX_ENTID']
