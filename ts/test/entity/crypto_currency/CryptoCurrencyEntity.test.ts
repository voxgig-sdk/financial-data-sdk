...
describe('CryptoCurrencyEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'crypto_currency.' + op, live)) return
    }
    ...
    const crypto_currency_ref01_data_dt0 = await crypto_currency_ref01_ent.load(crypto_currency_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['FINANCIAL_DATA_TEST_CRYPTO_CURRENCY_ENTID']
