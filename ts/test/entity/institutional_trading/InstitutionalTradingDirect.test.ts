...
describe('InstitutionalTradingDirect', async () => {
  ...
  test('direct-load-institutional_trading', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-institutional_trading', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FINANCIAL_DATA_TEST_INSTITUTIONAL_TRADING_ENTID']
