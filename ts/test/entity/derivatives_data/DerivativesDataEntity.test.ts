...
describe('DerivativesDataEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'derivatives_data.' + op, live)) return
    }
    ...
    const derivatives_data_ref01_data_dt0 = await derivatives_data_ref01_ent.load(derivatives_data_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['FINANCIAL_DATA_TEST_DERIVATIVES_DATA_ENTID']
