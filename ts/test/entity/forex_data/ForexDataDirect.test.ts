...
describe('ForexDataDirect', async () => {
  ...
  test('direct-load-forex_data', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-forex_data', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FINANCIAL_DATA_TEST_FOREX_DATA_ENTID']
