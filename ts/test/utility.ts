...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FINANCIALDATA_TEST_LIVE ||
    'TRUE' === process.env.FINANCIALDATA_TEST_OVERRIDE
  ) {
  ...
  m.FINANCIALDATA_TEST_EXPLAIN = process.env.FINANCIALDATA_TEST_EXPLAIN || m.FINANCIALDATA_TEST_EXPLAIN
